  ...
  >
    ...
    <div v-else-if="reportData" class="report-container">
      ...
      <div v-if="currentReportType === 'summary'" class="report-content">
        ...
        <!-- AI 总结与建议 -->
        <div class="report-section ai-summary-section">
          <h3 class="section-title">
            <el-icon color="#409EFF"><MagicStick /></el-icon> 
            AI 总结与建议
          </h3>
          <div class="ai-summary-content">
            <div v-if="reportData.overview.status === '已通过'" class="ai-success-msg">
              <p>🎉 <strong>测试执行顺利完成！</strong></p>
              <p>本次 AI 自动化测试用例共包含 {{ reportData.overview.total_steps }} 个步骤，均按预期正确执行完毕。</p>
              <p><strong>AI 建议：</strong> 您的业务功能表现稳定，可以考虑增加异常场景或边界测试用例来进一步提升测试覆盖率。</p>
            </div>
            <div v-else-if="reportData.overview.status === '执行失败'" class="ai-error-msg">
              <p>⚠️ <strong>测试执行过程中遇到异常！</strong></p>
              <p>AI 检测到有 {{ reportData.statistics.failed }} 个任务未能成功执行。</p>
              <p><strong>AI 诊断与建议：</strong></p>
              <ul>
                <li>请检查页面元素是否发生了变化，导致 AI 无法准确识别目标元素。</li>
                <li>网络延迟可能导致页面加载超时，请尝试增加前置等待时间。</li>
                <li>请查看下方的详细日志和截图，确认失败发生时的具体页面状态。</li>
              </ul>
            </div>
            <div v-else class="ai-info-msg">
              <p>ℹ️ <strong>测试执行被终止或未全部完成。</strong></p>
              <p>本次任务共有 {{ reportData.statistics.total }} 个任务，完成了 {{ reportData.statistics.completed }} 个。</p>
              <p><strong>AI 建议：</strong> 请检查任务是否被手动停止，或者测试流程是否存在阻塞环节。</p>
            </div>
          </div>
        </div>
...
import { Loading, VideoPlay, Download, MagicStick } from '@element-plus/icons-vue'
import { ElMessage } from 'element-plus'
import * as echarts from 'echarts'
import api from '@/utils/api'

const getAIExecutionReport = (id, params) => api.get(`/ui-automation/ai/execution-records/${id}/report/`, { params })
const exportAIExecutionReportPDF = (id, params) => api.get(`/ui-automation/ai/execution-records/${id}/export-report/`, { params, responseType: 'blob' })
...
.ai-success-msg { color: #67C23A; }
.ai-error-msg { color: #F56C6C; }
.ai-info-msg { color: #909399; }
